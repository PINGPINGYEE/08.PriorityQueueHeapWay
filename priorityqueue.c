...
void noHuffman(int freq[], char ch_list[], int n)
{
	int i;
	TreeNode* node, * x;
	HeapType* heap;
...
int main(int argc, char* argv[]) {
	element arrayList[MAX_SIZE] = { 34, 12, 76, 59, 32, 55, 88, 26, 16, 79, 34, 85, 29, 78, 41, 56, 86 };
	heap_sort(arrayList, MAX_SIZE); // 히프 정렬
	for (int i = 0; i < MAX_SIZE; i++) {
		printf("%d ", arrayList[i].key);
	}
	printf("\n");
	return 0;
}
